    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header card_header">
                <div class="row">
                    <div class="col-md-8">
                        <h4 class="card-title card_title"><i class="fab fa-gg-circle"></i> All Content Information</h4>
                    </div>
                    <div class="col-md-4 text-right">
                        <a href="{{url('dashboard/page')}}" class="btn btn-dark btn-md waves-effect btn-label waves-light card_btn"><i class="fas fa-plus-circle label-icon"></i>All Page</a>
                    </div>
                </div>
            </div>
            <div class="card-body">
                ...
                <table id="alltableinfo" class="table table-bordered table-striped table-hover dt-responsive nowrap custom_table">
                    ...
                    <tbody>
                      @foreach($all as $data)
                      <tr>
                          <td>{{$data->pageInfo->page_name}}</td>
                          <td>{{$data->content_title}}</td>
                          <td>{{$data->content_subtitle}}</td>
                          <td>
                            @if($data->content_image!='')
                              <img class="img-thumbnail img40" src="{{asset('uploads/content/'.$data->content_image)}}"/>
                            @else
